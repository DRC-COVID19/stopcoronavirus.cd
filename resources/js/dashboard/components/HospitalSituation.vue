...
<template>
  <b-container class="p-0">
    <b-row lg="12" class="p-0 no-gutters">
      <b-col cols="12" fluid>
        <div
          class="col-12 mb-2 row align-items-start px-2 no-gutters justify-content-start"
        >
          ...
          <b-row v-if="!isLoading" class="mb-2 align-items-start">
            <b-col lg="12" class="align-items-center">
              <h4>{{ hospital.name || 'Rapport global' }}</h4>
            </b-col>
            <b-col lg="12" class="align-items-start">
          ...
          >
            ...
            <export-excel
              :data="hospitalsSituationsDatasToExport"
              :name="fileName"
            ...
            >
              <div>
                <button
                  :disabled="
                    completedFormsAggregated.data.length === 0 || isLoading
                  "
            ...
            >
              <p>
                ...
                <span
                  ...
                  class="mode-agreggation"
                  >(somme)</span
                >
                <span v-else class="mode-agreggation">(moyenne)</span>
              </p>
            </div>
          </b-card>
        </b-row>
...
export default {
  ...
  computed: {
    ...
    fileName() {
      let name = 'Données';
      if (this.filterData.observation_start) {
        name += `_du_${this.filterData.observation_start}`;
      }
      if (this.filterData.observation_end) {
        if (this.filterData.observation_start) {
          name += `_au_${this.filterData.observation_end}`;
        } else {
          name += `_a_la_date_du_${this.filterData.observation_end}`;
        }
      }
    ...
    hospitalsSituationsDatasToExport() {
      const hospitalsSituationsData = this.completedFormsAggregated.data || [];

      return hospitalsSituationsData.flatMap((hospital) => {
    ...
    fieldsToExport() {
      const fields = new Set(
        this.hospitalsSituationsDatasToExport.flatMap((hospital) => {
          return Object.keys(hospital);
        })
      );
      return [...fields].reduce((a, v) => ({ ...a, [v]: v }), {});
    },
  },
  watch: {
    hospitalData() {
