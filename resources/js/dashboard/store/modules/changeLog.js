export default {
  ...
  actions: {
    createChangeLog({ state, commit, dispatch }, payload = {}) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .post('/api/dashboard/change-log', payload)
          .then(({ data }) => {
    ...
    updateChangeLog({ state, commit, dispatch }, payload = {}) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .put(`/api/dashboard/change-log/${payload.id}`, payload)
          .then(({ data }) => {
            dispatch('getListChangedLogs');
            resolve(true);
          })
    ...
    removeChangeLog({ state, commit, dispatch }, payload = {}) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .delete(`/api/dashboard/change-log/${payload.id}`, payload)
          .then(({ data }) => {
            dispatch('getListChangedLogs');
            resolve(true);
          })
    ...
    setChangeLogsRead({ state, dispatch }) {
      return new Promise((resolve, reject) => {
        ...
        axios
          .post('/api/dashboard/change-log/read')
          .then(({ data }) => {
            dispatch('getListChangedLogs');
            resolve(true);
          })
  ...
  getters: {
    getChangeLogNotRead(state) {
      ...
      return state.listChangeLogs.data.filter(
        (x) => x.notRead
      ) /*.sort((a, b) => {
