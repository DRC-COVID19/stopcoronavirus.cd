      ...
      <div v-else class="pb-5">
        <b-row>
          <b-col lg="8" class="d-flex container-preview-back w-100 mb-3">
            ...
            >
              <i class="fas fa-arrow-left mr-3" aria-hidden="true"></i> Revenir
              aux formulaire
            </b-link>
          </b-col>
        </b-row>
        <b-row>
          <b-col
            lg="8"
            class="d-flex justify-content-center container-preview-title-form w-100 mb-3 flex-wrap"
          >
            <h3 class="text-white text-center w-100">
              {{ $route.params.form_title.toUpperCase() || 'Formulaire' }}
            </h3>
            <h4 class="text-white text-align-center-right">
              Hôpital:{{ $route.params.hospital_name }}
            </h4>
          </b-col>
        </b-row>
        <b-row class="mt-4">
          <b-col lg="8">
            <b-card class="default-card first-card mb-5">
              <b-card-header>
                <h5 class="text-center conflict-title">
              ...
              <b-card-body>
                ...
                >
                  <b-form-radio
                    ...
                    class="text-red-keep"
                    @input="keepOldValue"
                    >Conserver la soumission de
                    {{
                      completedFormsHospitalForm
                        ? completedFormsHospitalForm[0].created_manager_name
                        : ''
                    }}, Enregistrer le
                    {{
                      moment(completedFormsHospitalForm[0].created_at).format(
                        'DD/MM/YYYY'
                      )
                    }}
                  </b-form-radio>
                  <b-form-radio
                    ...
                    @input="keepNewValue"
                    >Conserver la soumission,Enregistrer le
                    {{
                      moment($route.params.last_update).format('DD/MM/YYYY')
                    }}</b-form-radio
                  >
                </b-form-group>
              </b-card-body>
            ...
            <b-card class="text-center card-step form-display-conflict">
              <span>{{
                formSteps[0].form_steps[currentStep - 1].title.toUpperCase()
              }}</span>
            </b-card>
            <div
              v-if="
                formSteps[0].form_steps[currentStep - 1].form_fields.length > 0
              "
            >
              <b-card
                v-for="(field, index) in formSteps[0].form_steps[
                  currentStep - 1
                ].form_fields"
                :key="index"
        ...
        <b-row class="mt-4">
          <b-col
            lg="8"
            class="px-0 d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center"
          >
            ...
            <b-col
              lg
              class="d-flex flex-wrap align-items-center justify-content-center my-4 my-md-0"
            >
              <div
                class="mx-4 my-2 d-flex col align-items-center justify-content-center"
              >
                ...
                <small class="font-weight-bold"
                  >Etape {{ currentStep }} sur
                  {{ formSteps[0].form_steps.length }}</small
                >
              </div>
              <div class="my-2">
                <b-pagination
...
export default {
  ...
  methods: {
    ...
    getResponse(formFieldId) {
      return this.completedFormsHospitalForm[0].completed_form_fields.find(
        (field) => field.form_field_id === formFieldId
      );
    },
    getResponseHistories(formFieldId) {
      return this.$route.params.field_histories.find(
        (field) => field.form_field_id === formFieldId
      );
    },
    onNextStep() {
      ...
      this.formSteps[0].form_steps.forEach((step) => {
        step.form_fields.forEach((field) => {
          if (this.getResponse(field.id)) {
            this.completedForm.completed_form_field[field.id] =
              this.getResponse(field.id).value;
          }
        });
      });
      ...
      this.formSteps[0].form_steps.forEach((step) => {
        step.form_fields.forEach((field) => {
          if (this.getResponseHistories(field.id)) {
            this.completedForm.completed_form_field[field.id] =
              this.getResponseHistories(field.id).value;
          }
        });
      });
      this.completedForm = { ...this.completedForm };
    ...
    onComplete() {
      this.completedFormHistoryStore({
        ...
        created_manager_name:
          this.completedFormsHospitalForm[0].created_manager_name,
        created_manager_first_name:
          this.completedFormsHospitalForm[0].created_manager_first_name,
