        ...
        <b-col cols="12" class="mt-4">
          ...
          >
            ...
            <template v-slot:cell(statut)="data">
              <div
                v-if="data.item.diff_date < 2 && data.item.diff_date >= 0"
                class="d-flex justify-content-start align-item-center"
              >
                <span
                  class="badge badge-pill badge-statut"
              ...
              <div
                v-else-if="data.item.diff_date >= 2 && data.item.diff_date <= 3"
                class="d-flex justify-content-start align-item-center"
              >
                <span
                  class="badge badge-pill badge-statut"
                ...
                <span class="ml-4"
                  >Retard de {{ data.item.diff_date }} jours</span
                >
              </div>
              <div
                v-else-if="data.item.diff_date > 3"
                class="d-flex justify-content-start align-item-center"
              >
                <span
                  class="badge badge-pill badge-statut"
                ...
                <span class="ml-4"
                  >Retard de {{ data.item.diff_date }} jours
                </span>
              </div>
              <div
                v-else
            ...
            <template v-slot:cell(created_manager)="data">
              <span v-if="data.item.completed_forms[0]"
                >{{ data.item.created_manager_name }}
                {{ data.item.created_manager_first_name }}</span
              >
              <span v-else> - </span>
            </template>
            <template v-slot:cell(last_update)="data">
              <span v-if="data.item.last_update">{{
            ...
            <template v-slot:cell(form)="data">
              <span v-if="data.item.completed_forms[0]">{{
                data.item.completed_forms[0].form.title
              }}</span>
              <span v-else>-</span>
            </template>
