<template>
  <b-tabs class="mx-0">
    <b-tab
      title="Paramètrage"
      :active="title === 'Paramètrage'"
      @click="showBookMarks('settings')"
    >
      <b-row class="mx-0 h-100 w-100" lg="12">
        ...
        <b-col class="mx-0 w-100 mt-4" lg="12">
          <label for class="text-dash-color"
            ><b-badge class="px-2">Étape 1</b-badge> : Source des données
          </label>
          <v-select
        ...
        <b-col
          class="mt-1 d-flex justify-content-around actions-container"
          lg="12"
        >
          <div>
            <b-button
    ...
    <b-tab title="Bookmark" @click="showBookMarks('bookmark')">
      <b-row>
        <b-col class="mx-0 w-100 mt-4 bookmark__overflow" lg="12">
          ...
          >
            <b-list-group-item
              :active="bookmark.id === activeItem && visibleBookmark"
              style="cursor: pointer"
              class="d-flex justify-content-between"
              @click.prevent="selectedBookmark(bookmark)"
              >{{
                bookmark.name.charAt(0).toUpperCase() + bookmark.name.slice(1)
              }}
              <!--<b-button :variant="bookmark.id === activeItem?'danger':'outline-danger'" size="sm" @click="deleteBookmark(bookmark)">supprimer</b-button>--></b-list-group-item
            >
          </b-list-group>
        </b-col>
      </b-row>
