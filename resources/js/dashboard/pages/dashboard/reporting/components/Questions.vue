<template>
  <b-col class="mx-0 w-100" lg="12">
    <label for class="text-dash-color"
      ><b-badge class="px-2"> {{ step }} </b-badge> : {{ title }}</label
    >
    ...
    <b-row>
      <b-col class="w-100 mx-1 py-1 container-axe" lg="11">
          ...
          <b-form-group label="" v-slot="{ ariaDescribedby }">
            ...
            >
              <b-form-checkbox
                ...
                checked="1"
                v-show="showDate && isDataSourceSelected"
              >
                Date
              </b-form-checkbox>
            </b-form-checkbox-group>
            ...
            >
              <b-form-checkbox
                :value="{ name: 'Commune', type: 'township' }"
                v-show="showTownship && isDataSourceSelected"
              >
                Commune
              </b-form-checkbox>
            </b-form-checkbox-group>
            ...
            >
              <b-form-checkbox
                :value="{ name: 'Hopital', type: 'hospital' }"
                v-show="showHospital && isDataSourceSelected"
                >Hôpital
              </b-form-checkbox>
            </b-form-checkbox-group>
          </b-form-group>
        ...
        <b-form-group label="" v-slot="{ ariaDescribedby }">
          <label for class="text-dash-color text-primary">{{
            !isDataSourceSelected
              ? 'Veuillez selectionner une source de données'
              : 'Questions:'
          }}</label>
          <b-form-checkbox-group
            id="checkbox-group-2"
...
export default {
  ...
  computed: {
    ...
    showTownship() {
      return (
        this.except.filter((item) => item.type === 'township').length === 0
      );
    },
    showHospital() {
      return (
        this.except.filter((item) => item.type === 'hospital').length === 0
      );
    },
    showDate() {
      return this.except.filter((item) => item.type === 'date').length === 0;
    },
    formFieldsFiltered() {
