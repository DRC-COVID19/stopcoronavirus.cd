<template>
  <b-modal id="orderResponse" centered hide-footer hide-header>
    <b-card class="border-0">
      <div class="mb-4">
        <h2 class="lead text-center text-bold">Insertion du champ</h2>
      </div>
      <div>
        <b-form @submit.prevent="onSubmit" label-class="text-dash-color">
          <div id="input-group-1" class="form-group-radio">
            <b-form-radio
      ...
      if (this.form.form_field_order) {
        ...
        const index =
          this.formFields.findIndex(
            (formField) => formField.id === this.formToPopulate.id
          ) + 1;
