    ...
    <b-collapse id="collapse-form-field" class="mt-2" visible>
      <b-card-body>
        <b-form @submit.prevent="onSubmit" @reset="onReset">
          ...
          <b-form-group id="step-group" label="Etape*" label-for="step">
            <b-form-select
              id="step"
...
export default {
    ...
    formFieldsSorted() {
      return this.targetForm.form_fields
        ...
        .filter((formField) => {
          return (
            !this.rowFormField ||
            !this.rowFormField.id ||
            this.rowFormField.id !== formField.id
          );
        });
    },
  },
  watch: {
    rowFormField() {
      const formFieldOrder =
        this.formFieldsSorted.find(
          (formField) => formField.order_field > this.rowFormField.order_field
        )?.id || null;
      this.form = {
        ...this.rowFormField,
  ...
  methods: {
    ...mapActions(['formFieldTypeIndex', 'formFieldStore', 'updateFormField']),
    onSubmit() {
      this.form.rules = this.fieldWillBeRequired ? 'required' : '';
      this.form.form_id = this.targetForm.id;
      if (!this.form.form_field_order) {
        const MaxValue = this.targetForm.form_fields.flatMap(
          (x) => x.order_field
        );
        this.form.order_field =
          MaxValue.length && MaxValue.length > 0
            ? Math.max(...MaxValue) + 1
            : 1;
      }
      if (!this.updating) {
