<template>
  <div class="bug__container">
    <b-container class="mt-4 px-lg-5">
      <Loading v-if="isLoading" class="h-100" :message="message" />
          ...
          <b-card class="p-md-5 p-lg-5 bg-dash">
            ...
            >
              <b-form-group class="border-0 m-0">
                <label
                  id="input-group-3"
                  class="text-dash-color text-label"
                  for="input-3"
                >
              ...
              <div class="mb-4">
                <label for="dropzone" class="text-dash-color text-label"
                  >Joindre un fichier ( Optionnel )</label
                >
                <b-progress
                  :value="progress"
                  show-progress
                ...
                >
                  <div class="dropzone-custom-content">
                    ...
                    <h6 class="subtitle text-label">
                      Ajouter plusieurs images si vous le souhaitez
                    </h6>
                  </div>
                </vue2Dropzone>
              </div>
  ...
  data() {
    ...
    return {
      ...
      cloudinaryConfig: {
        preset: 'uzcpaoas',
        apiURL:
          'https://api.cloudinary.com/v1_1/www-kinshasadigital-com/image/upload',
  ...
  methods: {
    ...
    uploadSuccess(file, response) {
      try {
        ...
        return axios
          .post(this.cloudinaryConfig.apiURL, formData, {
        ...
        return new Promise((resolve, reject) => {
          this.bugTracker__addTask({ task: this.data })
            .then(({ data }) => {
              ...
              if (
                this.attachements.length !== undefined &&
                this.attachements.length !== 0
              ) {
                this.bugTracker__addAttachementsToTask({
                  task: data,
                  ...
                  .then(({ data }) => {
                    this.isLoading = false;
                    this.$bvToast.toast(
                      'Le problème a été soumis avec succèss',
                      {
                        title: 'Signaler un Problème',
                        appendToast: true,
                        variant: 'success',
