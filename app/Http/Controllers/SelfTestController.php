...
class SelfTestController extends Controller
{
  ...
  private $questions = [
    ...
    [
      'id' => 22,
      'q' =>
        'Aidez la riposte à réaliser un suivi épidémiologique en donnant votre province, ville, commune et quartier.',
      'r' => 9,
    ],
  ];
  public function seltTest(Request $request)
  {
    ...
    $isResultat = 2;
    return view(
      'covidTest.selft_test',
      compact('content', 'backUrl', 'isResultat')
    );
  }
      ...
      case '22':
        $validator = Validator::make(
          ...
          [
            ...
            'other_town.required_if' => __(
              'Le champ autre ville est obligatoire.'
            ),
            'township.required_if' => __(
              'Le champ commune est obligatoire quand la valeur du champ ville est Kinshasa.'
            ),
          ]
        );
        if ($validator->fails()) {
        ...
        $isResultat = 3;
        return view(
          'covidTest.selft_test_result',
          compact('resultat', 'isResultat')
        );
      case '1':
      default:
        $validator = Validator::make(
  ...
  public function result(array $responses)
  {
    try {
      ...
      if (
        $responses['cough'] == 1 ||
        $responses['sore_throat_aches'] == 1 ||
        $responses['agueusia_anosmia'] == 1
      ) {
        if ($this->hasPronostic($responses)) {
  ...
  {
    if (
      $responses['temperature_cat'] == 1 ||
      $responses['temperature_cat'] == 2 ||
      $responses['temperature_cat'] == 4
    ) {
      return true;
    }
    return false;
  ...
  public function hasPronostic(array $responses)
  {
    if (
      $responses['age'] >= 70 ||
      $responses['imc'] >= 30 ||
      $responses['heart_disease'] == 1 ||
      $responses['diabetes'] == 1 ||
      $responses['cancer'] == 1 ||
      $responses['breathing_disease'] == 1 ||
      $responses['kidney_disease'] == 1 ||
      $responses['liver_disease'] == 1 ||
      $responses['pregnant'] == 1 ||
      $responses['immunosuppressant_disease'] == 1 ||
  ...
  public function minorGravity(array $responses)
  {
    ...
    if (
      isset($responses['tiredness_details']) &&
  ...
  public function storeDiagnostic(array $responses, array $message)
  {
    ...
    $responses['immunosuppressant_disease_algo'] =
      $responses['immunosuppressant_disease'] == 1;
    ...
    $responses['immunosuppressant_drug_algo'] =
      $responses['immunosuppressant_drug'] == 1;
    if ($responses['heart_disease'] == 2) {
      $responses['heart_disease'] = 999;
    }
    $responses['heart_disease_algo'] =
      $responses['heart_disease'] == 1 || $responses['heart_disease'] == 999;
    $duration =
      strtotime(date('Y-m-d H:i:s')) - strtotime($responses['start_at']);
    $responses['duration'] = $duration;
    if ($responses['age'] < 15) {
      $responses['age_range'] = 'inf_15';
    } elseif ($responses['age'] >= 15 && $responses['age'] <= 49) {
      $responses['age_range'] = 'from_15_to_49';
    } elseif ($responses['age'] >= 50 && $responses['age'] <= 69) {
  ...
  public function ExtractGeoCoding(array $responses)
  {
    try {
      if (file_exists(storage_path('app/townGeocoding.json'))) {
        ...
        if (
          array_key_exists(
            strtoupper($responses['province'] . '_' . $responses['township']),
            $data
          )
        ) {
          return [
  ...
  public function updateDatabase()
  {
    try {
      ...
      foreach ($dataFromDb as $value) {
        if (
          !is_null($this->townGeocoding) &&
          $value->township &&
          array_key_exists(
            strtoupper($value->province . '_' . $value->township),
            $this->townGeocoding
          )
        ) {
          $value->longitude =
          ...
          $coordonne = $this->addTownGeoCoding(
            strtoupper($value->township),
            strtoupper($value->province)
          );
          if ($coordonne && is_array($coordonne) && count($coordonne) > 1) {
