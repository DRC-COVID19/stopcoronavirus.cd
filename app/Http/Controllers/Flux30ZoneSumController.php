...
class Flux30ZoneSumController extends Controller
{
  ...
  public function getHotspotMaps(Request $request)
  {
    ...
    try {
      ...
      $fluxObservations = Flux24ZoneHospot::select([
        'Observation_Zone',
      ...
      $fluxObservations = $fluxObservations
        ->whereBetween('date', [
      ...
      $fluxReferences = Flux24ZoneHospot::select([
        'Observation_Zone',
        DB::raw('sum("volume") as volume'),
      ]);
      if ($data['fluxGeoOptions'] != 'Tout') {
  ...
  public function getHotspotDaily(Request $request)
  {
    ...
    try {
      $data['fluxGeoOptions'] = $data['fluxGeoOptions'][0];
      $fluxObservations = Flux30ZoneSum::select([
        'Date as date',
        'Hour as hour',
      ...
      $fluxReferences = Flux30ZoneSum::select([
        'Date as date',
        'Hour as hour',
        DB::raw('sum("Volume") as volume, WEEKDAY("Date") as day'),
      ]);
      ...
      foreach ($fluxObservationGroup as $key => $observation) {
        ...
        foreach ($observation as $value) {
          $referenceHour = array_filter($reference, function ($item) use (
            $value
          ) {
  ...
  public function getHotspotDailyByDate(Request $request)
  {
    ...
    try {
      ...
      $fluxObservations = Flux24ZoneHospot::select([
        'date as date',
        DB::raw(
          '\'00:00\' as hour,sum("volume") as volume, WEEKDAY("date") as day'
        ),
      ]);
      if ($data['fluxGeoOptions'] != 'Tout') {
        $fluxObservations->where('Observation_Zone', $data['fluxGeoOptions']);
  ...
  public function getHotspotTendance(Request $request)
  {
    ...
    try {
      $data['fluxGeoOptions'] = $data['fluxGeoOptions'][0];
      $flux = Flux24ZoneHospot::select([
        'date as date',
        DB::raw('\'00:00\' as hour, sum("volume") as volume'),
      ]);
      if ($data['fluxGeoOptions'] != 'Tout') {
