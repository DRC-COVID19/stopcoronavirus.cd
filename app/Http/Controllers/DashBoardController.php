...
class DashBoardController extends Controller
{
  ...
  function getAllDiagnosticStat()
  {
    $datafromDb = DB::table('diagnostics')
      ->select([
        DB::raw(
          'COUNT(id) as count, orientation,CAST( created_at AS DATE) as created_at'
        ),
      ])
  ...
  {
    $sheetsValues = $this->readSheetValue(
      env('SONDAGE_SPREADSHEET_ID'),
      env('SONDAGE_SHEET_NAME')
    );
    $formattedArray = [];
    ...
    for ($i = 1; $i < $countArray; $i++) {
      ...
      if (isset($formattedArray[$key])) {
        ...
        if (
          $value[9] == 'Pas très inquiet' ||
          $value[9] == 'Un peu inquiet' ||
          $value[9] == 'Pas inquiet'
        ) {
  ...
  public function getFluxData(Request $request)
  {
    $data = Validator::make($request->all(), [
      ...
      'preference_end' =>
        'date|before:observation_start|required_with:preference_start',
    ...
    try {
      $flux = Flux::select([
        ...
        ->whereIn('Destination', $data['destination'])
        ->get();
      ...
      return response()->json(
        array_merge($fluxRefencesData, $fluxData),
        200,
        [],
        JSON_NUMERIC_CHECK
      );
  ...
  public function getFluxZone()
  {
    try {
      $zones = DB::select(
        'SELECT origin FROM flux_24 UNION SELECT destination AS origin FROM flux_24 '
      );
  ...
  public function getFluxProvinces()
  {
    try {
      $zones = DB::select(
        'SELECT "Origin" as origin FROM flux24_provinces UNION SELECT "Destination" AS origin FROM flux24_provinces'
      );
  ...
  public function getFluxDataFromOriginProvince(Request $request)
  {
    ...
    try {
      ...
      $fluxRefences = [];
      if (isset($data['preference_start']) && isset($data['preference_end'])) {
        $fluxRefences = Flux24Province::select([
          'origin',
          'destination',
          DB::raw('sum(volume) as volume'),
        ])
  ...
  public function getFluxDataFromOriginDailyProvinceCompare(Request $request)
  {
    ...
    try {
      $flux = DB::select(
        "SELECT origin, DATE as date , SUM(volume) AS volume
            FROM (
        ...
        [$data['preference_start'], $data['observation_end']]
      );
  ...
  public function getFluxDataPredefinedDailyCompare(Request $request)
  {
    ...
    try {
      $flux = DB::select(
        "SELECT origin, DATE as date , SUM(volume) AS volume FROM (
  ...
  public function getFluxDataPredefinedDailyIn(Request $request)
  {
    ...
    try {
      $flux = Flux::select([
        ...
        ->groupBy('Date', 'destination', 'Origin')
        ->get();
  ...
  public function getFlux30DataFromOrigin(Request $request)
  {
    ...
    try {
      ...
      $fluxRefences = [];
      if (isset($data['preference_start']) && isset($data['preference_end'])) {
        $fluxRefences = Flux30Zone::select([
          'origin',
          'destination',
          DB::raw('sum(volume) as volume'),
        ])
    ...
    try {
      ...
      foreach ($flux as $value) {
        ...
        if (isset($geoData[strtoupper($value->zone)][0])) {
          $value->{'position_start'} =
            $geoData[strtoupper($value->zone)][0]['coordinates'];
        } else {
          continue;
        }
        if (isset($geoData[strtoupper($value->zone)][0])) {
      ...
      return response()->json(
        array_merge($fluxRefencesData, $fluxData, 200, [], JSON_NUMERIC_CHECK)
      );
    } catch (\Throwable $th) {
      if (env('APP_DEBUG') == true) {
        return response($th)->setStatusCode(500);
  ...
  public function getTownshipsByForm($form_id)
  {
    try {
      $townships = Township::with([
        'hospital.completedForms' => function ($query) use ($form_id) {
          if ($form_id) {
