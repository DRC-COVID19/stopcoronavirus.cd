...
class NotificationUser implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $betweenMonth           = [Carbon::now()->startOfMonth(), Carbon::now()->endOfMonth()];
        $dayForms               = [];
        ...
        foreach($listOfRecurrenceForms as $listOfRecurrenceForm){
            ...
            ->whereHas('formRecurrence',function($q) use ($listOfRecurrenceForm){
                        ...
                        ->whereDoesntHave('notifications',function($q) use ($dateTimeCarbon, $listOfRecurrenceForm,$betweenWeek,$betweenMonth,$yearNotification){
            ...
            if($listOfRecurrenceForm == 'semaine')
            {
                $query = $query->where('form_recurrence_value','=', Carbon::now()->dayOfWeek); 
            }
            if($listOfRecurrenceForm == 'mois')
            {
                $query = $query->where('form_recurrence_value','=', Carbon::now()->format('j')); 
            }
            if($listOfRecurrenceForm == 'année')
            {
                $query = $query->where('form_recurrence_value','=', Carbon::now()->month); 
            }
            $results = $query->get();
        ...
        foreach($dayForms as $dayForm){
            Notification::create([
                ...
                'message'   => 'nous vous rappelons que vous devrez remplir le formulaire aujourd\'hui',
                'form_id'   =>  $dayForm->id
            ]);
