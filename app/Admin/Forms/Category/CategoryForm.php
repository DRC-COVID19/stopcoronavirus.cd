    ...
    public function handle(Request $request)
    {
        try {
            ...
            if (isset($this->all()['Info général']['id'])) {
                ...
                CategoryTranslation::create(array_merge($this->all()['Français'], ['category_id' => $category->id]));
                CategoryTranslation::create(array_merge($this->all()['Kikongo'], ['category_id' => $category->id]));
                CategoryTranslation::create(array_merge($this->all()['Lingala'], ['category_id' => $category->id]));
                CategoryTranslation::create(array_merge($this->all()['Swahili'], ['category_id' => $category->id]));
                CategoryTranslation::create(array_merge($this->all()['Tshiluba'], ['category_id' => $category->id]));
            }
            Db::commit();
            $this->clear();
