...
class DiagnosticController extends AdminController
{
  ...
  protected function grid()
  {
    ...
    $grid
      ->column('temperature_cat', __('Fièvre'))
      ->bool([
        'NSP' => true,
        'sup_39' => true,
        '37.8-38.9' => false,
    ...
    $grid
      ->column('breathlessness', __('Soufle'))
      ->bool(['1' => true, '0' => false])
      ->filter([
        0 => 'Non',
        1 => 'oui',
    ...
    $grid
      ->column('sore_throat_aches', __('Mal de gorge/courbatures'))
      ->bool(['1' => true, '0' => false])
      ->filter([
        0 => 'Non',
        1 => 'oui',
    ...
    $grid
      ...
      ->display(function () {
        if (
          ...
          $this['imc'] >= 30 ||
          $this['heart_disease_algo'] == 1 ||
          $this['diabetes'] == 1 ||
          $this['cancer'] == 1 ||
          $this['breathing_disease'] == 1 ||
          $this['kidney_disease'] == 1 ||
          $this['liver_disease'] == 1 ||
          $this['pregnant'] == 1 ||
          $this['immunosuppressant_disease_algo'] == 1 ||
  ...
  protected function detail($id)
  {
    ...
    $show
      ->{"temperature_cat"}(
        __('Fièvre ces 48 dernières heures (frissons, sueurs) ?')
      )
      ->as(function ($value) {
        $r = '';
        switch ($value) {
    ...
    $show->field(
      'sore_throat_aches',
      __(
        'Mal de gorge et/ou des douleurs musculaires et/ou des courbatures inhabituelles'
      )
    );
    $show->field(
      'diarrhea',
      __('Ces dernières 24 heures, avez-vous de la diarrhée')
    );
    $show->field('tiredness', __('Une fatigue inhabituelle'));
    $show->field('tiredness_details', __('Alitement > 50% du temps diurne'));
    $show->field(
      'feeding_day',
      __('Impossibilité de vous alimenter ou de boire DEPUIS 24 HEURES OU PLUS')
    );
    ...
    $show->field(
      'heart_disease',
      __(
        'hypertension artérielle mal équilibrée ? Ou avez-vous une maladie cardiaque ou vasculaire ? Ou prenez vous un traitement à visée cardiologique ?'
      )
    );
    $show->field('diabetes', __('Êtes-vous diabétique ?'));
    $show->field(
      'cancer',
      __('Avez-vous ou avez-vous eu un cancer ces trois dernières années ?')
    );
    $show->field(
      'breathing_disease',
      __(
    ...
    $show->field(
      'kidney_disease',
      __('Avez-vous une insuffisance rénale chronique dialysée ?')
    );
    $show->field(
      'liver_disease',
      __('Avez-vous une maladie chronique du foie ?')
    );
