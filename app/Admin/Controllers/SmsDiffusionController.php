...
class SmsDiffusionController extends AdminController
{
  ...
  protected function grid()
  {
    ...
    $grid
      ->column('provinces', __('Zone cibles'))
      ->display(function ($provinces) {
  ...
  {
    $adminUserSmsDiffusion = AdminUserSmsDiffusion::where(
      'admin_user_id',
      $admin_id
    )
      ->where('sms_diffusion_id', $diffusion_id)
      ->get()
      ->first();
  ...
  protected function form()
  {
    ...
    if (Admin::user()->can('validator_sms')) {
      $form
        ...
        ->options(['DRAFT' => 'Draft', 'VALIDATED' => 'Validé']);
    } else {
      $form
        ->select('status', __('Status'))
        ->options(['DRAFT' => 'Draft', 'VALIDATED' => 'Validé'])
    ...
    $form
      ->multipleSelect('provinces', __('Zones cibles'))
      ->options(Province::all()->pluck('name', 'id'));
    $form
      ->datetime('date_diffusion', __('Date diffusion'))
        ...
        foreach ($operators as $value) {
          if (isset($value->email)) {
            Mail::to($value->email)->send(
              new SmsChangeStatusMail($form->model())
            );
          }
        }
      }
